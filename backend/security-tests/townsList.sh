#!/bin/bash
# Updated test script for the townsList endpoint with additional test cases
...
if [ "$RATE_LIMIT_TEST" = true ]; then
    echo "Running rate limit test for townsList endpoint with test type '$TEST_TYPE'..."
    ...
    echo "Running test type '$TEST_TYPE' for townsList endpoint..."
    curl -ks $PROXY -X POST -d "$PAYLOAD" "$URL"
    echo
fi