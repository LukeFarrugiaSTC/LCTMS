#!/bin/bash
# Test the readRoles endpoint with optional rate limit test
...
# API_KEY is required for readRoles
if [ -z "$API_KEY" ]; then
    echo "API_KEY is not set in the .env file."
    exit 1
fi

URL="https://localhost:443/endpoints/readRoles.php"
PAYLOAD='{"api_key": "'"$API_KEY"'"}'

if [ "$RATE_LIMIT_TEST" = true ]; then
    echo "Running rate limit test for readRoles endpoint..."
